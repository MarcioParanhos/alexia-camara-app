...
import { TrendingUp, Users, Plus, Sparkles, FileText, UserPlus } from "lucide-react";
...
export default async function ProntuarioPage({ params }: { params: { id: string } }) {
  ...
  const paciente = await prisma.patient.findUnique({
    ...
    include: {
      ...
      attachments: { where: { evolutionId: null }, orderBy: { uploadedAt: "desc" } },
  ...
  return (
    <div className="rounded-2xl p-5 sm:p-8 bg-bg">
      <Link href="/dashboard" className="text-xs mb-6 flex items-center gap-1 text-inkFaint">
        ← Voltar ao painel
      </Link>
      ...
      <div className="grid lg:grid-cols-[280px_1fr] gap-6 lg:gap-8">
        ...
        <div className="rounded-xl p-6 h-fit bg-white border border-line">
          ...
          <h3 className="text-xl mt-4 mb-1 font-display font-semibold text-ink">{paciente.name}</h3>
          <p className="text-sm mb-5 text-inkFaint">{idade !== null ? `${idade} anos` : "Idade não informada"}</p>

          <div className="space-y-4 text-sm">
            <div>
              <p className="text-[11px] uppercase tracking-wide mb-1 text-inkFaint">Diagnóstico</p>
              <p className="text-ink">{paciente.diagnosis || "Não informado"}</p>
            </div>
            <div>
              <p className="text-[11px] uppercase tracking-wide mb-1 text-inkFaint">Início do tratamento</p>
              <p className="text-ink">{formatarData(paciente.startDate)}</p>
            </div>
            {adesao !== null && (
              <div>
                <p className="text-[11px] uppercase tracking-wide mb-1 text-inkFaint">Adesão ao plano</p>
                <div className="flex items-center gap-2">
                  <span className="flex-1 h-1.5 rounded-full overflow-hidden bg-surfaceAlt">
                    <span className="block h-full rounded-full bg-accent" style={{ width: `${adesao}%` }} />
                  </span>
            ...
            <div className="pt-2">
              <p className="text-[11px] uppercase tracking-wide mb-1.5 text-inkFaint">Familiares vinculados</p>
              <div className="flex items-center gap-2">
                <Link href={`/dashboard/pacientes/${paciente.id}/acessos`} className="flex items-center gap-1.5 text-xs text-inkSoft">
          ...
          <div className="pt-5 mt-5 border-t border-line">
            <p className="text-[11px] uppercase tracking-wide mb-2 text-inkFaint">Anexos do paciente</p>
            <p className="text-[11px] mb-2.5 text-inkFaint">
              Documentos e exames que não pertencem a uma sessão específica (laudos, avaliações posturais etc.).
            </p>
            <PainelAnexos patientId={paciente.id} initialAnexos={paciente.attachments} compact />
          </div>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-6 text-sm text-primary border-b border-line pb-3">
            <TrendingUp size={14} /> Evolução ({paciente.evolutions.length} registro{paciente.evolutions.length === 1 ? "" : "s"})
          </div>

          {paciente.evolutions.length === 0 ? (
            <div className="rounded-xl p-8 text-center bg-white border border-dashed border-line">
              <p className="text-sm text-inkFaint mb-4">Nenhum registro de evolução ainda.</p>
              <Link href={`/dashboard/pacientes/${paciente.id}/nova-evolucao`} className="text-sm text-primary underline">
            ...
            <div>
              {paciente.evolutions.map((ev) => (
                <div key={ev.id} className="rounded-xl p-4 sm:p-5 mb-4 bg-white border border-line">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2 mb-2">
                    <div>
                      <p className="text-[15px] font-display font-semibold text-ink">{ev.title}</p>
                    ...
                    <div className="flex items-center gap-4">
                      {ev.painLevel !== null && (
                        <div className="flex items-center gap-1.5">
                          <span className="text-lg leading-none font-mono text-ink">{ev.painLevel}</span>
                          <span className="text-[10px] text-inkFaint">/10 EVA</span>
                        </div>
                      )}
                      {ev.romDegrees !== null && (
                        <div className="text-right">
                          <p className="text-lg leading-none font-mono text-ink">{ev.romDegrees}°</p>
                          <p className="text-[10px] text-inkFaint">amplitude</p>
                        </div>
                      )}
                    </div>
                  </div>
                  <p className="text-sm leading-relaxed text-inkSoft whitespace-pre-line">{ev.note}</p>
                  {ev.highlighted && (
                  ...
                  <AnexosEvolucaoToggle patientId={paciente.id} evolutionId={ev.id} initialAnexos={ev.attachments} />
                </div>
              ))}
            </div>
          )}
